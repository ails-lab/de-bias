...
MERGE_LEFT = ['be', 'ge', 'her', 'ont', 'ver', 'on', 'wan']

# selection of common suffixes
MERGE_RIGHT = ['aar', 'er', 'eur', 'aard', 'erd', 'achtig', 'es', 'heid', 'erig', 'ig',
               'arij', 'nij', 'erij', 'dij', 'ij', 'in', 'ing', 'je', 'shap', 'sel', 'te',
               'tor', 'trix', 'teit', 'tie', 'ette', 'teur', 'trice', 'stan']

PLURAL_SUFFIX = ['en', 'n', 's', '\'s', 'EN', 'N', 'S', '\'S']

PLURAL_TRANSITIONS = {
    r'(?P<vowel>[aeou])(?P<consonant>[b-df-hj-np-tv-z])$': r'\g<vowel>\g<vowel>\g<consonant>',
    r'(?P<vowel>[aeiou])(?P<consonant>[b-df-hj-np-tv-z])(?P=consonant)$': r'\g<vowel>\g<consonant>',
    r'v$': 'f',
    r'z$': 's'
}
...
def compute_singular(item, ahocs):
    ...
    for suf in sorted(PLURAL_SUFFIX, reverse=True):
        ...
        if item.endswith(suf):
            ...
            # alternatively check for plural transitions and convey to singular forms, in case
            else:
                for plural_form, singular_form in PLURAL_TRANSITIONS.items():
                    if re.search(plural_form, item_singular):
                        repl_item = re.sub(plural_form, singular_form, item_singular)
                        if repl_item.lower() in ahocs:
                            return repl_item
